...
def test_ride_start_returns_200_and_payload(client, fleet_manager_mock: Mock) -> None:
    fleet_manager_mock.start_ride.return_value = Mock(
        ride_id=123,
        vehicle_id="V-7",
        vehicle_type="car",
        start_station_id=10,
    ...
    fleet_manager_mock.start_ride.assert_called_once_with(user_id=1, location=(34.8, 32.1))
...
def test_ride_start_maps_conflict_to_409(client, fleet_manager_mock: Mock) -> None:
    fleet_manager_mock.start_ride.side_effect = ConflictError("User already has an active ride")
    try:
        resp = client.post("/ride/start", json={"user_id": 1, "lon": 34.8, "lat": 32.1})
